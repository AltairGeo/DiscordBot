...
class moderation(commands.Cog):
    ...
    async def addwarn(self, ctx: discord.ApplicationContext):
        if await func.moder(ctx=ctx):
            await ctx.respond("Выбор пользователя:",
                              view=uui.TargetSelectView())
        else:
    ...
    async def addwarn_legacy(self,
        ...
        for i in author_roles:
            ...
            if str(i) == mod:
                user_id = name.id
                user_name = name.name
                await ctx.respond(await dswarn.add_warn(user_id,
                                                        user_name,
                                                        reason, loop))
    ...
    async def alluserwarn(self,
        ...
        if await func.moder(ctx):
            ...
            for i in await dswarn.all_user_warn(name.id, loop=loop):
                embed = discord.Embed(title=f"{i[2]}",
                                      description=f"Причина: {i[3]}",
                                      color=discord.Color.dark_green())
                embed.set_footer(text=f"warn_id: {i[0]}")
                ListOfMessage.append(await ctx.send(embed=embed,
                                     view=uui.AllUserWarns(i[0])))
            await asyncio.sleep(60)
            for i in ListOfMessage:
                try:
                    await i.delete()
                except Exception as e:
                    logging.error(f"alluserwarn delete messages"
                                  f" was crush! With error:{e}")
        else:
    ...
    async def mute(self,
        ...
        for i in author_roles:
            ...
            if str(i) == mod:
                ...
                embed = discord.Embed(title=f"{name.name}",
                                      description=f"Выдан"
                                      f" мут на {hours} часов.",
